...
while active_1:
    ...
    prompt_1 = input('Have you had an account ID?\
                    \nType \'Y\' for Yes, or \'N\' for No, or \'quit\' to exit the game\n')

    user_list = pd.read_csv('../saved_file/user_list.csv', 
    ...
    if prompt_1.lower() == 'y':
        active_2 = True
        while active_2:
            user_id = input('Enter your unique account ID, or \'back\' to go to back to account menu\n')
        ...
        active_3 = True
        while active_3:
            new_user_id = input('Enter your unique account ID, or \'back\' to go to back to account menu\n')
...
while True:
    if f"{user_id}.txt" in os.listdir('../saved_file'):
        ...
        if prompt_5.lower() == 'y':
            ...
            prompt_6 = input('Exit the game? Your progress this time is not saved.\
...
while True:
        ...
        if prompt.lower() == 'y':
            player_dictionary.update({'Mission 1': 'Not completed', 'Mission 2': 'Not completed'})
            key_list = ['position', 'fire position', 'fire power', 
                        'mask', 'e_type','e_power']
